...
namespace PkgLab
{
    public class WebProjectSystem : PhysicalFileSystem, IProjectSystem
    {
        ...
        public void RemoveReference(string name)
        {
            ...
            if (!GetFiles(BinDir, recursive: false).Any())
            {
        ...
        public void AddFrameworkReference(string name)
        {
            ...
            if (fullName == null)
            {
                throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, "UnknownFrameworkReference", name));
            }
        ...
        /// </summary>
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Justification = "We never want to throw from this method. If Assembly.Load fails, the only message we want to display is that package could not be installed.")]
        internal static string ResolvePartialAssemblyName(string name)
        {
            foreach (var key in _knownPublicKeys)
            {
                var assemblyFullName = String.Format(CultureInfo.InvariantCulture, "{0}, Version={1}, Culture=neutral, PublicKeyToken={2}", name, VersionUtility.DefaultTargetFrameworkVersion, key);
        ...
        {
            // In WebMatrix, if a file is present under the Website's directory
